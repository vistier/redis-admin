<?php
	...
	if($post['command']=="addkey-string" AND isset($post['key']) AND isset($post['value']) AND !empty($post['type'])){
	 	$DB->setNewKey($post);
	...
	<div class="left" style="margin: 10px 5px;">	
		...
		<div id="widget_1" class="widget">
			...
			<table>
				...
				<td><input type="text" name="value" width="20" value="<?php if($get['action']=='edit'){ 
				 	echo $DB->getSingleKeyValue($_SESSION['REDIS']['DATABASE'], $get['key']); 
					}?>" /></td>
				</tr>															
			</table>
